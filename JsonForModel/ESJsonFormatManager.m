...
+ (NSString *)formatObjcWithKey:(NSString *)key value:(NSObject *)value classInfo:(ESClassInfo *)classInfo{
    ...
    if ([value isKindOfClass:[NSString class]]) {
        return [NSString stringWithFormat:@"\n/** <#description#> */\n@property (nonatomic, %@) %@ *%@;",qualifierStr,typeStr,key];
        ...
        if ([firstObj isKindOfClass:[NSDictionary class]]) {
            ...
            if (childInfo.isModel==ModelType_Json) {
        ...
        if ([ESJsonFormatSetting defaultSetting].useGeneric && [ESUtils isXcode7AndLater]) {
            return [NSString stringWithFormat:@"\n/** <#description#> */\n@property (nonatomic, %@) %@%@ *%@;",qualifierStr,typeStr,genericTypeStr,key];
        }
        return [NSString stringWithFormat:@"\n/** <#description#> */\n@property (nonatomic, %@) %@ *%@;",qualifierStr,typeStr,key];
...
+ (NSString *)parseClassHeaderContentForOjbcWithClassInfo:(ESClassInfo *)classInfo{
    ...
    if (classInfo.isModel==ModelType_Json) {
        result= [NSMutableString stringWithFormat:@"@interface %@ : JSONModel\n",classInfo.className];
    }else{
