...
#import "ESPair.h"
@interface HomeVC ()<NSTextViewDelegate,NSWindowDelegate>
/**
...
@property (nonatomic, strong) IBOutlet NSTextView *inputPlistUrl;
@property (nonatomic, strong) IBOutlet NSScrollView *jsonTextView;

@property (nonatomic, strong) IBOutlet NSTextView *inputTextView;

@property (nonatomic, strong) IBOutlet NSButton *enterButton;

@property (nonatomic, strong) IBOutlet NSButton *cancelButton;

@property (nonatomic, strong) IBOutlet NSScrollView *scrollView;
@property (nonatomic, strong) IBOutlet NSButton *help;

@end

@implementation HomeVC

- (IBAction)yy_btnAction:(NSButton *)sender {
...
- (IBAction)enterButtonClick:(NSButton *)sender {
    ...
    if (plistUrlStr.length > 2 ) {
        ...
        //        result = [NSDictionary dictionaryWithContentsOfURL:plistUrl];
        NSString*string = [[NSString alloc]initWithContentsOfFile:plistUrlStr];
        
        if (string.length<2) {
            NSError *error = [NSError errorWithDomain:@"地址或文件有错误" code:0 userInfo:nil];
            NSAlert *alert = [NSAlert alertWithError:error];
            [alert runModal];
        }else{
            result = [self dictionaryWithJsonStr:string];
            return;
        ...
        NSString *string = self.inputTextView.string ;
        
        result = [self dictionaryWithJsonStr:[string convert]];
    ...
    if ([result isKindOfClass:[NSDictionary class]]) {
        ...
        if ([ESJsonFormatSetting defaultSetting].outputToFiles) {
            ...
            [dialog setDataWithMsg:msg defaultClassName:ESRootClassName enter:^(NSString *className) {
                ...
                if (self.mj_btn.state) {
                    classInfo.isModel=ModelType_MJ;
                    
                }else if(self.yy_btn.state){
                    classInfo.isModel=ModelType_YY;
                }else{
                    classInfo.isModel=ModelType_Json;
                }
            }];
    ...
    for (NSString *key in dic) {
        ...
        if ([obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSDictionary class]]) {
            ...
            if ([obj isKindOfClass:[NSDictionary class]]) {
                ...
                if (self.mj_btn.state) {
                    childClassInfo.isModel=ModelType_MJ;
                    
                }else if(self.yy_btn.state){
                    childClassInfo.isModel=ModelType_YY;
                }else{
                    childClassInfo.isModel=ModelType_Json;
                }
